...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            rppuz[i+9*j] = RangeProof(32);
            rppuz[i+9*j].in <== puzzle[i][j];
            rppuz[i+9*j].range[0] <== 0;
            rppuz[i+9*j].range[1] <== 1;

            assert (rppuz[i+9*j].out == 1);

            rpsol[i+9*j] = RangeProof(32);
            rpsol[i+9*j].in <== solution[i][j];
            rpsol[i+9*j].range[0] <== 0;
            rpsol[i+9*j].range[1] <== 1;
            assert (rpsol[i+9*j].out == 1);
